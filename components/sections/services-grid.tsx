        ...
        >
          {services.map((s) => (
            <StaggerItem key={s.id} className="h-full">
              <Link
                href={`/layanan/${s.slug}`}
                className="group flex h-full flex-col rounded-2xl border border-white/10 bg-white/[0.04] p-6 transition-all duration-300 hover:-translate-y-1 hover:border-brand-orange/40 hover:bg-white/[0.07]"
              >
                <span className="inline-flex size-12 items-center justify-center rounded-xl bg-brand-orange/15 text-brand-orange ring-1 ring-brand-orange/20 transition-colors group-hover:bg-brand-orange group-hover:text-white">
