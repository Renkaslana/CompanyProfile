...
import { AlertTriangle, CheckCircle2, Images, Plus } from "lucide-react";
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-6">
      ...
      {items.length === 0 && !query ? (
        <EmptyState
          icon={Images}
          title="Belum ada item galeri"
          description="Foto operasional yang Anda tambahkan akan tampil di halaman publik /galeri. Pastikan media telah diunggah ke Media Library terlebih dahulu."
          action={canWrite ? { label: "Tambah item pertama", href: "/admin/gallery/new" } : undefined}
        />
      ) : items.length === 0 ? (
        <EmptyState
          mode="no-match"
          icon={Images}
          title={`Tidak ada item galeri cocok dengan "${query}".`}
          reset={{ label: "Bersihkan pencarian", href: "/admin/gallery" }}
        />
      ) : (
